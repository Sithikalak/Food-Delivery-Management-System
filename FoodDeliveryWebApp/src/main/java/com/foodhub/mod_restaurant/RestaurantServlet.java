...
import com.foodhub.mod_billing.ReviewDAO;
        ...
        try {
            if ("add".equals(action)) {
                ...
                if (user == null || (!"ADMIN".equals(user.getRole()) && (res == null || res.getOwnerId() != user.getUserId()))) {
                    response.sendRedirect("mod_customer/login.jsp?error=Unauthorized");
                    return;
                }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if ("list".equals(action)) {
                ...
                request.getRequestDispatcher("mod_restaurant/restaurant-list.jsp").forward(request, response);
                ...
                if (user != null && "CUSTOMER".equals(user.getRole())) {
                    request.getRequestDispatcher("mod_restaurant/menu-view.jsp").forward(request, response);
                } else {
                    request.getRequestDispatcher("mod_restaurant/menu-manage.jsp").forward(request, response);
                }
            } else if ("search".equals(action)) {
                request.setAttribute("restaurants", restaurantDAO.searchRestaurants(request.getParameter("query")));
                request.getRequestDispatcher("mod_restaurant/restaurant-list.jsp").forward(request, response);
            }
        } catch (SQLException e) {
            throw new ServletException(e);
